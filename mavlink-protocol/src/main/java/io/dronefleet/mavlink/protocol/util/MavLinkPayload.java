...
public class MavLinkPayload {
  ...
  public void getByteArray(byte[] dst,int len){
    payload.position(index);
    if (payload.limit() - index < len){
      payload.get(dst,0,payload.limit() - index);
      index += payload.limit() - index ;
      return;
    }
    payload.get(dst,0,len);
    index += len;
  }
